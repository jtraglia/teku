...
public class GossipProposerSlashingTestExecutor implements TestExecutor {
  ...
  public void runTest(final TestDefinition testDefinition) throws Throwable {
    ...
    final ProposerSlashingValidator validator =
        new ProposerSlashingValidator(spec, recentChainData);

    // Process each message
    for (final Message message : metaData.messages) {
      final ProposerSlashing slashing = loadProposerSlashing(testDefinition, message.message);
  ...
  private ProposerSlashing loadProposerSlashing(
      final TestDefinition testDefinition, final String messageName) {
    return TestDataUtils.loadSsz(
        testDefinition, messageName + SSZ_SNAPPY_EXTENSION, ProposerSlashing.SSZ_SCHEMA);
  }
