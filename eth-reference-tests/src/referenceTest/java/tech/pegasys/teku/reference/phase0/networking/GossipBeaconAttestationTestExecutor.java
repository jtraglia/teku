...
public class GossipBeaconAttestationTestExecutor implements TestExecutor {
  ...
  public void runTest(final TestDefinition testDefinition) throws Throwable {
    ...
    for (final Message message : metaData.messages) {
      ...
      final UInt64 messageTimeMs = genesisTimeMillis.plus(metaData.currentTimeMs).plus(offsetMs);
      forkChoice.onTick(messageTimeMs, Optional.empty());

      final Attestation attestation = loadAttestation(testDefinition, spec, message.message);

      final ValidatableAttestation validatableAttestation =
          message.subnetId != null
              ? ValidatableAttestation.fromNetwork(spec, attestation, message.subnetId)
      ...
      assertValidationResult(
          message.expected, message.reason, result, attestation.hashTreeRoot().toHexString());
    }
  }
  ...
  private Attestation loadAttestation(
