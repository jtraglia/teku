...
public class GossipVoluntaryExitTestExecutor implements TestExecutor {
  ...
  public void runTest(final TestDefinition testDefinition) throws Throwable {
    ...
    for (final Message message : metaData.messages) {
      final SignedVoluntaryExit exit = loadVoluntaryExit(testDefinition, message.message);
      final SafeFuture<InternalValidationResult> resultFuture = validator.validateForGossip(exit);

      assertThat(resultFuture).isCompleted();
      final InternalValidationResult result = safeJoin(resultFuture);

      assertValidationResult(message.expected, result, exit.hashTreeRoot().toHexString());
    }
  }
  ...
  private SignedVoluntaryExit loadVoluntaryExit(
      final TestDefinition testDefinition, final String messageName) {
    return TestDataUtils.loadSsz(
        testDefinition, messageName + SSZ_SNAPPY_EXTENSION, SignedVoluntaryExit.SSZ_SCHEMA);
  }
